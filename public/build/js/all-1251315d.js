...
/*
	NOTE: This version of Gritter has been edited to alter the HTML to better match 
		the Bootstrap & Labcoat styles.
*/
...
(function($){
	...
	*/
	var Gritter = {
		...
		_tpl_close: '<button type="button" class="gritter-close-nrl close" aria-label="Close"><span aria-hidden="true">×</span></button>',
		...
		*/
		add: function(params){
			...
			*/
			$(item).find('.gritter-close-nrl').click(function(){
				Gritter.removeSpecific(number, {}, null, true);
				return false;
			});
			return number;
...
$(function() {
	...
	$('li.has-sub-menu .title-wrapper').on('click',function(){
		...
		if($(this).parent().hasClass('open')) {
			$.ajax({
				type: "GET",
				url: '/menuitems/'+menuItemId+'/status',
				data: {status: 'closed'},
				success: function(data) {
					// console.log('hazzah!'+data);
				}
			});
		} else {
			$.ajax({
	...
	//
	// This sets up buttons or anchors that are supposed to show modals.
	...
	$('body').on('click', 'button[data-modal-url], a[data-modal-url]', function() {
		...
		if ($(this).data('modal-id').length) {
			// Is the modal already in the DOM?
			if($('#'+modalId).length) {
	...
	$(".impersonate").click(function() {
		...
		$.ajax({
			...
			data: data,
			error: function(data) {
				displayErrorGritter(data.responseJSON);
			},
			success: function(data) {
				location.reload();
			}
...
// Use primarly for the trash can in a table row.
// Any anchor/button with data-delete-url set will hide the row it's in
// and send the delete to the server via ajax
function deleteSetup() {
	$('a[data-delete-url], button[data-delete-url]').each(function(){
		var target = this;
		if (!$(target).hasClass('popConfirmSet')){

			$(target).on('click', function() {
				var url = $(target).data('delete-url');
				var data = {'_token':$('input[name="_token"]').val(),'_method' : 'DELETE'};

				// Check for custom data delete element
				var element = $(target).data('delete-element');
				if (typeof element != 'undefined') {
					var $tr = $(target).closest(element);
				} else {
					// A tr is the default and most common use case
					var $tr = $(target).closest('tr');
				}

				// Remove the row to make things responsive to the user
				$tr.slideUp();
				...
				$.ajax({
					...
					data: data,
					error: function(data) {
						// Return the row because there was an error
						$tr.slideDown();
						displayErrorGritter(data.responseJSON);
					},
					success: function(data) {
						// The row has been deleted on the server
						$tr.remove();
					}
				});
			})
...
// Pass this the modalId as a string.  The modalId is required.
// If you want to use more options, pass in an options object in the following format:
// { modalId: 'string', success: function() { //do some stuff }, error: function() { // do some different stuff } }
function modalAjaxSetup(options) {
	var modalId = '';
	if (typeof options == 'string') {
		modalId = options;
	...
	if (modalId == '') {
		return "Error: The required modalId was not set";
	}
	...
	// 	if (event.which == 13){
	// 		event.preventDefault();
	// 		$("button[name='default-success']").click();
	// 	}
	// });

	// Set up the success button
	$modalId.find('[name="default-success"]').click(function() {
		$button = $(this);
		$button.addClass('ajax-active');

		// Get key elements from the form
		...
		$.ajax({
			...
			error: function(data) {
				...
				if (typeof options['error'] == 'function') {
					...
					displayError($modalId.find('.errorMessage'), messages);
				}
			},
			success: function(data) {
				// Turn off the ajax button
				$button.children('.ajax-indicator').popover('hide');
				$button.removeClass('ajax-active');
				$modalId.find('.errorField').removeClass('errorField');
...
function displayError($div, messages) {
	...
	$div.html(html).slideDown();
}

// {"label":["The label field is required."],"name":["The name field is required."]}
// {"Error":["This is the error message"]}
function displayGritter(messages) {
	for (var key in messages) {
		var text = '';
		for (var line in messages[key]) {
			text += messages[key][line]+'<br />';
		}
		$.gritter.add({
			title: key,
			text: text
